...
import { Buttons, PlayerEvent, PlayerState, Theme } from './Player';
...
class ControlsClass {
  private readonly props: IControlProps;
  ...
  public render() {
    const { instance, playerState, state, setLoop, setSeeker, play, pause, stop, buttons } = this.props;
    ...
    const showPlayButton = !buttons || buttons.includes(Buttons.Play);
    const showStopButton = !buttons || buttons.includes(Buttons.Stop);
    const showRepeatButton = !buttons || buttons.includes(Buttons.Repeat);
    const showFrameInput = !buttons || buttons.includes(Buttons.Frame);
    const showBackgroundChange = !buttons || buttons.includes(Buttons.Background);
    const showSnapshot = !buttons || buttons.includes(Buttons.Snapshot);
    const ICON_SIZE = { width: 14, height: 14, viewBox: '0 0 24 24' };

    const bg = this.props.theme === Theme.Transparent
      ? 'transparent'
      : this.props.theme === Theme.Dark ? '#3C3C3C'
      : this.props.theme === Theme.Light ? '#ffffff' : '#fff';

    return (
      <div
        class="lottie-player-controls"
        style={{
          'display': 'flex',
      ...
      >
        <Show when={showPlayButton}>
          <div
            ...
            class="lottie-player-btn"
            style={ControlButtonStyle}
          >
            <Show when={playerState === PlayerState.Playing}>
              <svg {...ICON_SIZE}>
        ...
        <Show when={showStopButton}>
          <div
            ...
            onClick={() => stop && stop()}
            onKeyDown={() => stop && stop()}
            class={playerState === PlayerState.Stopped ? 'lottie-player-btn active' : 'lottie-player-btn'}
            style={ControlButtonStyle}
          >
            <svg {...ICON_SIZE}>
              <path
        ...
        <Seeker
          ...
          onChange={(newFrame: any) => {
            if (setSeeker) { setSeeker(newFrame, false); }
          }}
        ...
        />
        <Show when={showFrameInput}>
          <div role="button" class="lottie-player-btn-container">
            <input
              style={{
                'outline': 'none',
                'border': this.props.theme === Theme.Dark ? '1px #505050 solid' : '1px #ccc solid',
                'border-radius': '3px',
                'width': '40px',
                'text-align': 'center',
                'background-color': this.props.theme === Theme.Dark ? '#505050' : '#ffffff',
                'color': this.props.theme === Theme.Dark ? '#B9B9B9' : '#999',
                'font-size': '0.7rem',
                'padding': '0',
                'font-family': 'inherit',
              }}
        ...
        <Show when={showRepeatButton}>
          <div
            ...
            class={state().loop ? 'lottie-player-btn active' : 'lottie-player-btn'}
            style={ControlButtonStyle}
          >
            <svg {...ICON_SIZE}>
              <path
        ...
        <Show when={showBackgroundChange}>
          <Popover
            icon={
              <svg {...ICON_SIZE}>
                <path
          ...
          >
            <div slot="content" class="lottie-popover popover-background">
        ...
        <Show when={showSnapshot}>
          ...
          >
            <div
              slot="content"
              class="lottie-popover lottie-popover-snapshot"
              onWheel={e => {
                if (setSeeker) {
                  setSeeker(state().seeker + (e.deltaY > 0 ? -1 : 1), false);
                }
              }}
            >
              <h5>Frame {state().seeker}</h5>
              <div style={{ cursor: 'pointer', color: '#0FCCCE' }} onClick={this.props.snapshot}>
                Download
              </div>
              <i class="lottie-note">Scroll with mousewheel to find exact frame</i>
            </div>
          </Popover>
        </Show>
      </div>
